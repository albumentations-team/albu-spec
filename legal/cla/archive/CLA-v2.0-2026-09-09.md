# albu-spec Contributor License Agreement
...
albu-spec, publish it under
AGPL-3.0-only, and offer separately licensed commercial or proprietary versions.
An accepted contribution remains available in the public project under
AGPL-3.0-only even when it is also licensed under other terms.

This version applies only after You expressly accept **Version 2.0**. Merely
changing the CLA file does not bind a previous contributor to new terms. When
You accept Version 2.0, the grants cover Your qualifying contributions from
before, on, and after the acceptance date as described below.
...
6. **Material** means the albu-spec project and any work based on or
   incorporating a Covered Contribution.
7. **Submit** means intentionally sending material to a repository, issue
   tracker, code-review system, mailing list, or other channel managed by or for
   Albumentations for the purpose of discussing or improving the Material. For
   an Individual Acceptance, You perform that act. For an Entity Acceptance, a
   Covered Contributor performs that act within the scope and covered period
   identified in the Entity Acceptance Record.
8. **Acceptance Date** means the timestamp recorded in the Acceptance Record.
...
worldwide, non-exclusive, transferable, irrevocable,
royalty-free, fully paid-up patent license, with the right to sublicense through
multiple tiers, to make, have made, use, offer to sell, sell, import, and
otherwise transfer the Covered Contribution and the Covered Contribution in
...
Albumentations may license Covered Contributions, alone or as part of the
Material, under AGPL-3.0-only and under separately negotiated commercial,
proprietary, or other terms. The grants in Sections 4 and 5 expressly authorize
that dual-licensing model.

As a condition of exercising the right to license a Covered Contribution under
any terms other than AGPL-3.0-only, including permissive, source-available,
commercial, proprietary, or other terms, Albumentations will first or
simultaneously make the accepted version of that Covered Contribution available
as part of a public albu-spec revision under AGPL-3.0-only. Removal from a
later release does not revoke the AGPL license already granted for a public
revision.
...
claim that Version 2.0 was accepted before the Acceptance Date.
...
Each CLA version is a separate offer. Acceptance of Version 1 does not by
itself constitute acceptance of Version 2.0 or a later version. Albumentations
will preserve the operative text and an identifier for each version in
`legal/cla/archive/` and preserve Acceptance Records outside release artifacts.

The exact Version 2.0 acceptance methods are listed in the **Acceptance
Instructions** below. Albumentations may offer another method only if the
resulting record identifies Version 2.0 and captures the information required
...
have legal capacity to accept this Agreement. If not, do not use the pull-request
comment; contact `vladimir@albumentations.ai` for a separate capacity and
parent-or-guardian process appropriate to Your circumstances.

After reading this exact Version 2.0, comment on the pull request:

```text
I have read and agree to the albu-spec CLA Version 2.0 (September 9, 2026) as an individual.
```

The Acceptance Record must retain the comment, GitHub identity, timestamp, and
the Version 2.0 identifier from `legal/cla/archive/MANIFEST.md`.
...
the albu-spec CLA Version 2.0 (September 9, 2026).
```
